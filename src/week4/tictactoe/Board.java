...
public class Board {
    ...
    public boolean isField(int index) {
        boolean isfield = (0 <= index && index < DIM * DIM);
        return isfield;
    }
    ...
    public boolean isField(int row, int col) {
        boolean isfield = (0 <= row && row < DIM) && (0 <= col && col < DIM);
        return isfield;
    }
    ...
    public Mark getField(int row, int col) {
        Mark field = fields[index(row, col)];
        return null;
    }
    ...
    public boolean isEmptyField(int i) {
        boolean isempty = (this.getField(i) == Mark.EMPTY);
        return isempty;
    }
    ...
    public boolean isEmptyField(int row, int col) {
        boolean isempty = (this.getField(index(row, col)) == Mark.EMPTY);
        return isempty;
    }
    ...
    public boolean gameOver() {
        boolean gameover = this.isFull() || this.hasWinner();
        return gameover;
    }
    ...
    public boolean hasRow(Mark m) {
        ...
        for (int i = 0; i < DIM; i++) {
            boolean hasrow2 = true;
            for (int j = 0; j < DIM; j++) {
                if (!(fields[(index(i, j))] == m)) {
                    hasrow2 = false;
                }
            }
            if (hasrow2) {
    ...
    public boolean isWinner(Mark m) {
        boolean iswinner = this.hasRow(m) || this.hasColumn(m) || this.hasDiagonal(m);
        return iswinner;
    }
    ...
    public boolean hasWinner() {
        boolean haswinner = isWinner(Mark.XX) || isWinner(Mark.OO);
        return haswinner;
    }
