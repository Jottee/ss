...
public class Card {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        if (args.length != 0) {
            ...
            writer = new PrintWriter("cardfile.txt");
        }
        card1.write(writer);
        card2.write(writer);
    }
    ...
    public static Card read(BufferedReader in) throws EOFException {
        ...
        if (suit == null || rank == null) throw new EOFException("Suit and/or rank is null");
        boolean containsSuit = false;
        for (char c : SUIT_CHARACTERS) {
            if (c == suitString2Char(suit)) {
                containsSuit = true;
                break;
            }
        }
        boolean containsRank = false;
        for (char c : RANK_CHARACTERS) {
            if (c == rankString2Char(rank)) {
                containsRank = true;
            }
        }
        if (containsRank && containsSuit) {
    ...
    public Card write(DataOutput out) {
        try {
    ...
    public Card write(ObjectOutput out) {
        try {
